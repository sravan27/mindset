#\!/bin/bash
# Remote fix script for EC2

echo "Creating MINDSET directory structure..."
mkdir -p /home/ubuntu/Mindset/src/ml/silicon_layer
mkdir -p /home/ubuntu/Mindset/src/rust/metrics_engine/src
mkdir -p /home/ubuntu/Mindset/src/frontend/pages
mkdir -p /home/ubuntu/Mindset/src/api

# Create basic files for the Silicon Layer
cat > /home/ubuntu/Mindset/src/ml/silicon_layer/__init__.py << 'EOF1'
# Silicon Layer Module
EOF1

cat > /home/ubuntu/Mindset/src/ml/silicon_layer.py << 'EOF1'
"""
...
cat > /home/ubuntu/Mindset/src/rust/metrics_engine/Cargo.toml << 'EOF1'
[package]
name = "metrics_engine"
version = "0.1.0"
edition = "2021"
...
cat > /home/ubuntu/Mindset/src/rust/metrics_engine/src/lib.rs << 'EOF1'
use pyo3::prelude::*;
...
cat > /home/ubuntu/Mindset/src/rust/metrics_engine/build.py << 'EOF1'
#\!/usr/bin/env python
"""
...
cat > /home/ubuntu/Mindset/src/rust/metrics_engine/py_wrapper.py << 'EOF1'
"""
...
cat > /home/ubuntu/Mindset/src/frontend/package.json << 'EOF1'
{
...
cat > /home/ubuntu/Mindset/src/frontend/pages/index.js << 'EOF1'
...
# Create a basic API
cat > /home/ubuntu/Mindset/src/api/main.py << 'EOF1'
"""
...
cat > /home/ubuntu/Mindset/run_pipeline_aws.py << 'EOF1'
#\!/usr/bin/env python
"""
...
chmod +x /home/ubuntu/Mindset/run_pipeline_aws.py
chmod +x /home/ubuntu/Mindset/src/rust/metrics_engine/build.py

# Fix permissions
chown -R ubuntu:ubuntu /home/ubuntu/Mindset

# Fix .env file
cat > /home/ubuntu/Mindset/.env << 'EOF1'
# AWS Settings
...
cd /home/ubuntu/Mindset
source venv/bin/activate || python3 -m venv venv && source venv/bin/activate
pip install fastapi uvicorn

echo "Fix complete. Restarting services..."
systemctl restart mindset-api.service
systemctl restart mindset-frontend.service
systemctl restart mindset-pipeline.timer

echo "Service status:"
systemctl status mindset-api.service
systemctl status mindset-frontend.service
systemctl status mindset-pipeline.timer
